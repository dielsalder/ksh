# Uses residual output from linalg.lstsq instead of provided res function
...
def lstsq_res(a, b, void):
	return float(linalg.lstsq(a, b)[1])

class crdset:
	def __init__(self,crds):
		self.a = build_amat(crds)
		self.b = build_bmat(crds)
		self.x = solve(crds)
		self.lstsq_res = float(linalg.lstsq(self.a, self.b)[1])
		self.res = res_x(crds, self.x)
		self.radius = radius(self.x)
		self.phi_min = float(fmin_bfgs(lstsq_res, 0, args = (crds, 0), disp = 0))
		self.the_min = float(fmin_bfgs(res_the, 0, args = (crds, self.phi_min), disp = 0))

incrds = crdset(crds)
print incrds.lstsq_res
