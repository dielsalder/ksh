...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   !B matrix
   ! construct matrix for B in linear problem
   ! for iatom = 1; iatom < nr; iatom++
   DO IATOM = 1,NR
      ! element in array for each rotation
      newXYZ_BMAT(IATOM) = newX(IATOM)*newX(IATOM) + newY(IATOM)*newY(IATOM)
   END DO
   
   !A matrix
   ! is different in organization from the B matrix
   newXYZ_AMAT(:,1) = 1
   DO IATOM = 1,NR
...
!  Back-out the actual parameters, R, X0, and Y0 from the fit results
   ...
   do while(task(1:2).eq.'FG'.or.task.eq.'NEW_X'.or. & task.eq.'START'
	   call setulb ( n, m, x, l, u, nbd, res, g, factr, pgtol, &
		       wa, iwa, task, iprint, &
    ...
    RES = RES + ABS( &
                     & (newX(I)-X0)*(newX(I)-X0) + &
                     & (newY(I)-Y0)*(newY(I)-Y0) + &
		     ! the radius has to be calculated first
		     ! inputs newX, newY and R_SVD
                     & (R_SVD)*(R_SVD)           - &
                     & (2*R_SVD)*( SQRT (          &
                     & (newX(I)-X0)*(newX(I)-X0) + &
                     & (newY(I)-Y0)*(newY(I)-Y0) ) ) )
...
!  Calculate the residual as per Vageli >>>
   RES = 0.0D+00
   DO I = 1,NR
   ! this is what should be minimized
   ! variables newX and newY
   ! where does svd fit in?
    RES = RES + ABS( &
                     & (newX(I)-X0)*(newX(I)-X0) + &
                     & (newY(I)-Y0)*(newY(I)-Y0) + &
		     ! the radius has to be calculated first apparently
                     & (R_SVD)*(R_SVD)           - &
                     & (2*R_SVD)*( SQRT (          &
                     & (newX(I)-X0)*(newX(I)-X0) + &
                     & (newY(I)-Y0)*(newY(I)-Y0) ) ) )
